...
export default function NoiTuTiengAnhPage() {
  ...
  const currentPrefixChar = lastMessage ? lastMessage.word.trim().slice(-1).toUpperCase() : "";
  ...
  const handleLevelWin = useCallback(() => {
    ...
    sounds.playFanfare();
    setIsBotThinking(false);
    completeEnLevel(activeLevel.id, 3, 100);
    setShowVictoryModal(true);
  }, [activeLevel, completeEnLevel]);
  ...
  const triggerBotTurn = useCallback(
    async (prevWord: string, currentHistory: ChainMessage[], level: WordChainLevel) => {
      ...
      const botRes = await GeminiAI.getAiWord("en", prevWord, usedWordsList);
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    const typed = playerInput.trim().toLowerCase();
    if (!typed) return;

    let fullWord = typed;
    if (!typed.toUpperCase().startsWith(currentPrefixChar)) {
      fullWord = `${currentPrefixChar.toLowerCase()}${typed}`;
    }

    setIsSubmitting(true);
    const usedWords = messages.map((m) => m.word);
    const evalResult = await GeminiAI.evaluateWord("en", fullWord, lastMessage?.word || null, usedWords);

    if (!evalResult.valid) {
      sounds.playWrong();
      setInputHasError(true);
      setErrorMessage(`${evalResult.error || "Invalid English word!"} (${timeLeft}s remaining to retry)`);
    ...
    const newMsg: ChainMessage = {
      ...
      meaning: evalResult.meaning || "Valid English word",
      sender: "player",
    };
  ...
  const handleNextLevel = () => {
    if (!activeLevel) return;
    const nextLvl = ENGLISH_CHAIN_LEVELS.find((l) => l.id === activeLevel.id + 1);
    if (nextLvl) {
      startLevel(nextLvl);
    } else {
      setActiveLevel(null);
        ...
        <div className="glass-card w-full rounded-[32px] p-6 bg-background/60 backdrop-blur-md space-y-5">
          <div className="flex items-center justify-between border-b border-border/50 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-black text-foreground">Nối Từ Tiếng Anh (English Chain)</h1>
                <p className="text-xs text-muted-foreground">Progressive Word Chain Levels</p>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3.5">
            {ENGLISH_CHAIN_LEVELS.map((level) => {
              const isUnlocked = !!enLevels[level.id];
              const stars = enLevels[level.id]?.stars || 0;
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <span className="text-xs font-black text-foreground">Level {level.id}</span>
                    <span className="flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 text-[10px] font-black">
                      <Gem className="h-3 w-3 text-blue-500" /> +{level.gemsReward}
                    </span>
                  </div>

                  <div className="my-2">
                    <p className="text-xs font-bold text-muted-foreground">{level.opponentName} ({level.targetWords} words)</p>
                    <p className="text-[11px] text-blue-700 dark:text-blue-300 font-black mt-0.5 truncate">
                      &quot;{level.starterWord}&quot;
                    </p>
                  </div>
        ...
        <div className="flex flex-col gap-3 w-full max-w-2xl mx-auto">
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <div className="text-center">
              <h2 className="text-base font-black text-foreground">Level {activeLevel.id}</h2>
            ...
            <button
              ...
              className="flex items-center gap-1 text-sm font-bold text-foreground hover:opacity-80 cursor-pointer"
            >
              <Flag className="h-4 w-4" /> Surrender
            </button>
          </div>
          ...
          <div className="relative flex items-center justify-between gap-3 px-4 py-2.5 rounded-2xl bg-white/90 dark:bg-gray-900/90 border border-border/60 shadow-sm">
            <span className="text-xs font-black text-foreground shrink-0">
              {completedWords}/{targetWords} words
            </span>
          ...
          {/* Timeline Box with Thinking Bubble */}
          <div className="relative min-h-[260px] max-h-[360px] overflow-y-auto p-4 rounded-[28px] bg-white/95 dark:bg-gray-900/95 border border-border/60 shadow-sm space-y-3.5 wordfight-scrollbar">
            ...
            {isBotThinking && (
              <div className="flex items-start gap-2.5 justify-start animate-in fade-in duration-150">
                ...
                <div className="p-3.5 rounded-[22px] rounded-tl-none bg-muted/50 border border-border/60 text-xs font-bold text-muted-foreground flex items-center gap-2">
                  <span>{activeLevel.opponentName} is thinking</span>
            ...
            <div className="sticky bottom-0 flex justify-end pointer-events-none">
              <button
                type="button"
                onClick={() => setErrorMessage("Enter a valid English word starting with the required letter.")}
          ...
          <button
            ...
            onClick={async () => {
              if (showHintBox) {
                ...
                const hints = await GeminiAI.getAiWord("en", lastMessage?.word || null, messages.map((m) => m.word));
                if (hints) {
                  setHintText(`Hint: ${hints.word} (${hints.meaning})`);
                } else {
                  setHintText("No hints available.");
                }
              }
            }}
          ...
          >
            ...
            <span>Show Word Hint (Gemini AI)</span>
          </button>

          {showHintBox && hintText && (
            <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/30 text-xs text-blue-700 dark:text-blue-300 font-bold animate-in fade-in">
              💡 {hintText}
            </div>
          )}
          ...
          <div className="flex items-center justify-between text-xs px-1 font-bold text-muted-foreground">
            <span className="flex items-center gap-1">⌨️ Enter your word below</span>
          ...
          {/* Input Form with Prefix and Circular Button */}
          <form onSubmit={handleSubmit} className="flex items-center gap-2.5">
            ...
            >
              ...
              <input
                ...
                placeholder={currentPrefixChar ? "..." : "Enter word..."}
            ...
            <button
              type="submit"
              disabled={currentTurn !== "player" || !playerInput.trim() || isSubmitting}
      ...
      {showSurrenderModal && activeLevel && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-sm rounded-[32px] bg-background p-6 text-center shadow-2xl border border-border space-y-4">
            ...
            <h3 className="text-lg font-black text-foreground">Do you want to surrender?</h3>
            <p className="text-xs text-muted-foreground">You will forfeit Level {activeLevel.id}.</p>
      ...
      {showDefeatModal && activeLevel && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in zoom-in-95">
          <div className="relative w-full max-w-sm rounded-[32px] bg-background/95 p-6 text-center shadow-2xl border-2 border-rose-500 space-y-4">
            ...
            <h3 className="text-xl font-black text-foreground">TIME UP! 😢</h3>
            <p className="text-xs text-muted-foreground">You didn&apos;t reach the {activeLevel.targetWords} words target for Level {activeLevel.id}.</p>
            <div className="flex gap-2 pt-2">
              <button
                type="button"
                onClick={() => setActiveLevel(null)}
      ...
      {showVictoryModal && activeLevel && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in zoom-in-95">
          <div className="relative w-full max-w-sm rounded-[32px] bg-background/95 p-6 text-center shadow-2xl border-2 border-[#7fe36a] space-y-4">
            ...
            <div>
              <h3 className="text-xl font-black text-foreground">LEVEL {activeLevel.id} COMPLETED! 🎉</h3>
              <p className="text-xs text-muted-foreground mt-1">Successfully connected {activeLevel.targetWords} words!</p>
            </div>
            ...
            <div className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center gap-2">
              ...
              <span className="text-sm font-black text-emerald-600">+{activeLevel.gemsReward * 2} Gems</span>
            </div>
